(() => {
  ...
  function moveForward() {
    ...
    if (!elementToMoveFrom || !elementToMoveTo)
      return;
  ...
  // src/app/readerModules/generateStory.js
  var toc = document.querySelector("#sequence-toc");
  ...
  function renderPlans(plans, toc2, story2) {
    let firstPlan = "";
    plans.forEach((plan, index) => {
      ...
      toc2.insertAdjacentHTML(
        "beforeend",
        `<li ${index == 0 ? `class="selected"` : ""} id="link-${plan.id}"><a class="" href="#plan-${plan.id}">${index + 1}</a></li>`
      );
      story2.insertAdjacentHTML(
        "beforeend",
        `<article ${plan.attributes.delay ? `data-story-delay="${plan.attributes.delay}"` : ""} data-strap-id="${plan.id}" class="plan" id="plan-${plan.id}">
        ${previousPlan ? `<a class="previousPlan" href="${previousPlan}">\u2190</a>` : ""}
...
//# sourceMappingURL=hypercomicsreader.js.map
