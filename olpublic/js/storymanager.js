(() => {
  ...
  async function createProject() {
    ...
    formElement.addEventListener("submit", (e) => {
      ...
      axios_default.post(`${config_default.strapi.url}/api/projects/`, { data }).then(function(response) {
        return response;
      }).catch((error) => {
        console.log(error);
      });
  ...
  async function loadAllProjects(serverUrl) {
    return axios_default.get(`${serverUrl}/api/projects?populate=deep`).then((response) => {
      return response;
    }).catch((err) => {
      return err;
    });
  ...
  async function removeSequenceFromProject(projectId, sequenceId) {
    await axios_default.put(`${config_default.strapi.url}/api/sequences/${sequenceId}`, {
      data: {
        project: {
  ...
  // src/app/api/api-sequences.js
  async function createSequence(projectId) {
    return await axios_default.post(`${config_default.strapi.url}/api/sequences/?populate=deep`, {
      data: {
        project: projectId,
  ...
  // src/app/storymanager.js
  start();
  var projectsList = document.querySelector("#projects-list");
  var projectSequence = document.querySelector("#projectSequences");
  ...
  async function renderProject(project) {
    ...
    projectsList.insertAdjacentHTML(
      ...
      )}</datetime> <a href="#project${project.id}">${project.attributes.title}</a> </li> `
    );
  ...
  <button  data-projectid="${project.id}" class="createSequence" >Add a sequence</button>
